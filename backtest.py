...
def handle_data(date):
    ...
    today_data=today_data[today_data['yoypni']>0]
    
    # 降序排列所选数据
    today_data['value']=today_data['yoypni']
    # 选出标的
...
plt.fill_between(x,max(ret.rev),min(ret.rev),where=((x<=(drawdown_end))&(x>=(drawdown_start))),facecolor='lightsteelblue',alpha=0.4) 
plt.legend()
