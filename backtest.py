...
#         data=ssdata.get_data(secid=stock,start_date=date,end_date=date,field=['yoyop'])
...
# 每月第一个交易日列表
trade_days=list(ssdata.get_data(secid='600036.sh',start_date=start_date,end_date=end_date,field='open').index)
start_date=trade_days[0].strftime('%Y-%m-%d')
end_date=trade_days[-1].strftime('%Y-%m-%d')
# 基准为新三板指数'899001.CSI'
benchmark = ssdata.get_data(secid='600036.sh' ,start_date=start_date,end_date=end_date,field='open')#基准
# 设置起始资金、股票仓位、现金仓位
capital = [1000000]
position = 0.
cash = capital[0]
