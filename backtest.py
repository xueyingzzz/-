...
def handle_data(date):
    ...
    # 选出20只标的
    today_data = today_data.sort_values('value',ascending = False)[:20]
    # 买入列表
    #buylist=list(today_data['CODES'])
    buylist=data_list
...
# 每月第一个交易日列表
days=['2014-01-02', '2014-02-07', '2014-03-03', '2014-04-01', '2014-05-05', '2014-06-03', '2014-07-01', '2014-08-01', '2014-09-01', '2014-10-08', '2014-11-03', '2014-12-01', '2015-01-05', '2015-02-02', '2015-03-02', '2015-04-01', '2015-05-04', '2015-06-01', '2015-07-01', '2015-08-03', '2015-09-01', '2015-10-08', '2015-11-02', '2015-12-01', '2016-01-04', '2016-02-01', '2016-03-01', '2016-04-01', '2016-05-03', '2016-06-01', '2016-07-01', '2016-08-01', '2016-09-01', '2016-10-10', '2016-11-01', '2016-12-01', '2017-01-03', '2017-02-03', '2017-03-01', '2017-04-05', '2017-05-02', '2017-06-01', '2017-07-03', '2017-08-01', '2017-09-01', '2017-10-09', '2017-11-01', '2017-12-01', '2018-01-02', '2018-02-01', '2018-03-01', '2018-04-02', '2018-05-02', '2018-06-01']
# 定义回测区间内的交易日列表
trade_days = days[days.index(start_date):days.index(end_date)] 
...
for date in (trade_days):
    ...
    if n % freq == 0 :
        ...
        for stock in list(h_amount.index):
            stock_data=ssdata.get_data(secid=stock,start_date=date,end_date=date,field=['tafactor','open'])  
    ...
    if drawdown > history_max:
        drawdown_start=target1.index[capital.index(max(capital[:-1]))]
        drawdown_end = target1.index[-1]
...
ret.to_csv('回测结果.csv')
ret['rev'].plot(color='firebrick',label='Alg')
#ret['benchmark'].plot(color='royalblue',label='基准收益')
n=len((ret.index).values)
x_tick = []
for i in range(n-1):                      # 遍历日期数目
    if i%(2) == 0:                                            # 每三十个交易日期生成一个图例
        x_tick.append((ret.index).values[i])
    else: 
        x_tick.append("")                                  # 其余图例为空值        
plt.xticks(range(len(x_tick)), x_tick,rotation = -90,fontsize=9)
