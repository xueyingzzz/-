...
def handle_data(date):
    ...
    # 选取处于创新层的股票
    today_data=ssdata.get_data(secid=data_list,start_date=date,end_date=date,field=['CREATIVECLASSORNOT','CAGRPNI'])
    today_data=today_data[today_data['CREATIVECLASSORNOT']== '是']
    # 去掉没有数据的股票
    today_data=today_data[np.isnan(today_data['CAGRPNI']) == False]

    # 降序排列所选数据
    today_data['value']=today_data['CAGRPNI']
...
# 每月第一个交易日列表
trade_days=['2013-07-01', '2014-01-02', '2014-02-07', '2014-03-03', '2014-04-01', '2014-05-05', '2014-06-03', '2014-07-01', '2014-08-01', '2014-09-01', '2014-10-08', '2014-11-03', '2014-12-01', '2015-01-05', '2015-02-02', '2015-03-02', '2015-04-01', '2015-05-04', '2015-06-01', '2015-07-01', '2015-08-03', '2015-09-01', '2015-10-08', '2015-11-02', '2015-12-01', '2016-01-04', '2016-02-01', '2016-03-01', '2016-04-01', '2016-05-03', '2016-06-01', '2016-07-01', '2016-08-01', '2016-09-01', '2016-10-10', '2016-11-01', '2016-12-01', '2017-01-03', '2017-02-03', '2017-03-01', '2017-04-05', '2017-05-02', '2017-06-01', '2017-07-03', '2017-08-01', '2017-09-01', '2017-10-09', '2017-11-01', '2017-12-01', '2018-01-02', '2018-02-01', '2018-03-01', '2018-04-02', '2018-05-02', '2018-06-01']
# 基准为创业板指数
...
for date in (trade_days):
    if n % freq == 0 :
        ...
        for stock in list(h_amount.index):
            stock_data=ssdata.get_data(secid=stock,start_date=date,end_date=date,field=['pb','open'])  
    ...
    ret=ret.append(pd.DataFrame({
                                 'rev': (capital[-1] - capital[0])/capital[0],
                                    'benchmark':(benchmark.loc[i,'OPEN' ]-benchmark['OPEN' ][0])/benchmark['OPEN' ][0],
                                 'max_drawdown': history_max},index =[i]))
...
# print ('基准收益',('%.2f%%' % (ret.iloc[-1].benchmark * 100)))
#print ('策略收益',('%.2f%%' % (ret.iloc[-1].rev * 100)))
# print ('策略年化收益',('%.2f%%' % (((1+ret.iloc[-1].rev)**(250/len(ret['date'].values))-1)* 100)))
#print ('最大回撤',('%.2f%%' % (ret.iloc[-1].max_drawdown * 100)))
# print ('最大回撤区间从',drawdown_start,'到',drawdown_end)
