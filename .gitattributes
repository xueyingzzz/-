...
def handle_data(date):
    ...
    # 选取处于创新层的股票
    today_data=ssdata.get_data(secid=data_list,start_date=date,end_date=date,field='CREATIVECLASSORNOT')
    selected=today_data[today_data['CREATIVECLASSORNOT']== '是']

    # 降序排列所选数据
    selected['value']=selected['CAGRPNI']
    selected = selected.sort_values('value',ascending = False)[:20]
    # 检查是否处理过数据
    buylist=list(selected['CODES'])
